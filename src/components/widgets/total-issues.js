...
class TotalIssues extends Component {
    ...
    componentDidMount() {
        if ( this.props.chartInput && this.props.chartInput.id ) {
            this.getTotalIssues( this.props.chartInput );
        }
    }

    componentWillReceiveProps( nextProps ) {
        if ( nextProps.chartInput && this.props.chartInput.id) {
            this.getTotalIssues( nextProps.chartInput );
        }
    }

    getTotalIssues( _input ) {
        this.setState( {
            isLoading: true,
            isError: false
        });
    ...
    render() {
        ...
        return (
            <div style={{boxShadow: "-1px 1px 16px rgba(0,0,0,.2)", minHeight:"350px", maxHeight:"350px"}}>
                <h2 className="heading-colors">Status By Device</h2>
...
export default connect( mapStateToProps, mapDispatchToProps )( TotalIssues );
